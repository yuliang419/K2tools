...
try:
	t,f,k,ra,dec = read_pixel(epic,6,'s')
except IOError:
	print 'FITS file does not exit'
...
for rad in apers:
	...
	tcorr1,f_corr1,xc1,yc1,raw1 = fit_lc(t1,ftot1,ftot,xc,yc,tsegs,tref,xref,yref)

	# t2,ftot2,segs2 = spline(t,ftot,tsegs,squiggles='mixed')
	# tcorr2,f_corr2,xc2,yc2 = fit_lc(t2,ftot2,xc,yc,tsegs,tref,xref,yref,plot=False)

	t3,ftot3,segs3 = spline(t,ftot,tsegs,squiggles=False)
	tcorr3,f_corr3,xc3,yc3,raw3 = fit_lc(t3,ftot3,ftot,xc,yc,tsegs,tref,xref,yref)
	...
	if rad<start_aper:
		dlim = max([min(best_f) - min_mad,0.98])
